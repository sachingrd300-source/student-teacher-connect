...
import { FileText, AlertTriangle, BookOpenCheck, Calendar, Users, Video, MapPin, Download, BookUp } from 'lucide-react';
...
const materialIcons: Record<string, JSX.Element> = {
  ...
  'Test Paper': <BookUp className="h-5 w-5 text-purple-500" />,
  Solution: <FileText className="h-5 w-5 text-green-500" />,
};

const GeneratedTestViewer = ({ material }: { material: StudyMaterial}) => {
    if (!material.questions) return null;
        ...
        <Dialog>
            ...
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>This is a test paper generated by your tutor.</DialogDescription>
                </DialogHeader>
                <div className="max-h-[70vh] overflow-y-auto pr-4 space-y-6">
                    {material.questions.map((q, index) => (
                         <div key={index}>
                                    ...
                                    {q.questionType === 'mcq' && q.options && (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 text-sm">
                                            {q.options.map((opt: string, i: number) => (
                                                <div key={i} className={`p-2 rounded-md ${opt === q.correctAnswer ? 'bg-green-500/10 text-green-700 font-medium' : 'bg-muted/50'}`}>
                            ...
                            {index < material.questions!.length - 1 && <Separator className="mt-6"/>}
                        </div>
                    ))}
                </div>
            </DialogContent>
                ...
                <Table>
                    ...
                    {materials?.map((material) => (
                        ...
                        <TableCell className="text-right">
                           {material.type === 'Test Paper' && material.questions ? (
                                <GeneratedTestViewer material={material} />
                            ) : (
                                <Button asChild variant="outline" size="sm" disabled={!material.fileUrl}>
